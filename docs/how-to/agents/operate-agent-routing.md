...
1. Confirm the Issue's `lgfc-issue-pr-contract-status:v1` comment shows `valid` at the current contract revision (re-apply `status:pr-ready` and wait for #2620's validator first if not — see the known limitation in step 2 before assuming this ever fires automatically).
2. **Known limitation, confirmed live by #2622's Phase 3 pilot:** neither `issue-pr-contract-validate.yml` nor `ops-agent-routing-controller.yml` exists on `main` (the default branch) as of this writing. GitHub only resolves non-PR-event triggers — `issues: [labeled]`, and the **filename**-addressed `workflows/{filename}/dispatches` REST endpoint (what `gh workflow run <file>.yml` and the GitHub web UI's "Run workflow" dropdown both use) — from the workflow file version on the default branch. Until these files are promoted to `main`:
   - `status:pr-ready` will **not** trigger #2620's validator automatically; and
   - dispatching by filename 404s (`POST .../workflows/ops-agent-routing-controller.yml/dispatches` → `404 Not Found`).

   Dispatch by the workflow's **numeric ID** instead — resolve it once via `GET /repos/{owner}/{repo}/actions/runs/{any_known_run_id}` (the `workflow_id` field in the response) or `GET /repos/{owner}/{repo}/actions/workflows` if the workflow has run at least once, then `POST /repos/{owner}/{repo}/actions/workflows/{numeric_id}/dispatches` with `ref: component/issue-contract-draft-pr` (or the current target component branch) and the same `mode`/`authorize_mutation`/`issue_number` inputs. This is how #2622's pilot actually fired it.
...
4. Known limitation, also confirmed live: a PR opened this way uses the default `GITHUB_TOKEN`, so GitHub will not automatically run other `pull_request`-triggered gates (`pr-hygiene`, `diff-scope`, `gitleaks`, `quality`, and the rest of the usual PR check suite) against it — #2622's pilot PR #2949 triggered none of them. Push an empty commit or close/reopen the PR to trigger them until a GitHub App installation token is adopted (`docs/reference/ci/issue-pr-contract.md` §7).
5. Known friction, found live: the Issue's PMO-intake `Delivery model:` value must be the strict literal `B-child` (etc.) that `delivery_profile.mjs` expects — the commonly-used phrasing `Model B child` fails classification. Use the literal form, matching PR-body convention, even though many existing Issues use the longer phrasing.

...
Owns: Durable LGFC roles, current member mapping, approval authority, protected stops, operating modes, launch-control workflow boundaries, and delegated task-closeout role boundaries
...
Related Issues: #2494, #2640, #2641, #2648, #2700
...
- Human relay is the least-desired fallback when the canonical channel is unavailable or Product Authority intervention is intentionally required.
- Any externally relayed decision must be written back to GitHub before repository work depends on it.
...
8. attempted bypass of Sandbox -> Development -> Promotion Candidate -> Production progression; or
9. attempted task closeout without required independent review, integration, post-integration verification, or deterministic terminal state.

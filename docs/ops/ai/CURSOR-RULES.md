...
For a graduated Project or Program, the exact prepared child graph is standing authority. Cursor self-claims the next eligible `team:operations`, `team:pmo`, or `team:governance` child one task at a time without routine Administration/PMO redispatch. Record starting SHA, branch, allowlist confirmation, and pre-implementation checkpoint before editing.

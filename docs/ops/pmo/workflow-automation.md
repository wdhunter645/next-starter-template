...
Owns: PMO workflow automation planning authority, Cursor queue-control boundaries, PR readiness rules, wave/run control concepts, and Program 5 promotion handoff for workflow automation
...
Represent the LGFC Workflow Automation design in GitHub documentation authority for the current Program 1 cycle: **PMO Automation and Agent Workflow Control**.

This document converts Workflow Automation from Program 5 ideas/project-draft material into bounded repository-owned planning authority. It defines what the automation program may design before any workflow code, label mutation, issue mutation, or runtime implementation begins.

This document is subordinate to `/docs/ops/pmo/PMO-V2-OPERATING-MODEL.md`.
...
- Program 5 promotion requirements for workflow automation candidates.
...
- issue `#1411` promotes Workflow Automation from Program 5 ideas/project-draft material into a new Program 1 planning cycle.
- legacy issue `#1379` captured Workflow Automation as future candidate material and listed available draft concepts.
- Program 2 under `#1255` remains active and must not be blocked, relabeled, closed, or otherwise mutated by this Program 1 planning work.
- Completed Program 1 cycle `#1335` remains historical evidence only. It is not the parent issue for this new Program 1 cycle.
...
- Program 1 produces production-ready task definitions for later issue alignment after Atlas/Bill walkthrough.
...
- Program 5 remains the ideas/project-draft lane for work not yet promoted into Programs 1-4.

## Program 1 Project Areas

| Area | Program 1 design output | Implementation boundary |
| --- | --- | --- |
| PMO v2 authority | Durable language that Programs 1-4 are rotating portfolio execution/planning lanes and Program 5 is ideas/project drafts | No issue or label mutation |
...
| Program 5 promotion process | Criteria for moving ideas/project drafts into Programs 1-4 | No child implementation issues from this PR |

## Workflow Automation Design Decisions

### Program lanes

Programs 1-4 are rotating execution/planning lanes in the LGFC prioritized execution portfolio. They are not permanent subject domains. Each Program 1-4 cycle implements a selected group of prioritized projects.

Program 5 is not an implementation lane. It is the ideas and project-drafts lane for concepts that still need documentation, readiness review, prioritization, and promotion before they enter Programs 1-4.

### Execution modes and queue control

LGFC programs use one-task handoff mode or launched-program queue mode. See `/docs/reference/pmo/lgfc-program-queue-and-dependency-map.md`. Launched programs require an approved dependency map before issue creation. Queue mode governs which task is authorized next; it does not grant Cursor merge, close, relabel, or queue-mutation authority.
...
A Program 5 item may feed a future Program 1-4 cycle only when:

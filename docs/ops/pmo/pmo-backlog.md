...
- **issue `#1411` is closed** and served its planning/control purpose as a completed planning artifact (`status:complete`). It is not an open or automatically queued program. Issues `#1417`–`#1424` are stale historical evidence only.
...
- **Priority #1 Website Completion / Fan Club Product Buildout** has a parked program issue and task chain (#1685 through #1694). It remains blocked from implementation until Bill/ChatGPT explicitly launch or reprioritize it.
- **Website Public Launch / Relaunch Readiness** has program issue #2039 and child issue chain #2041 through #2048. It follows Program #1685 structural baseline closeout and remains blocked until explicit Bill/ChatGPT task assignment.
...
- **Priority #2 Fundraiser / Charity Campaign Operations Buildout** has program issue #1700 and child issue chain #1701 through #1708. Task 001 assignment guidance is posted on #1701. It remains queued behind Program #1255/#1259 and parked Priority #1 unless Bill/ChatGPT explicitly reprioritize.
- **Priority #3 PMO Governance / Workflow Automation Completion** has program issue #1719 and child issue chain #1720 through #1727. **Construction complete on component branch** (2026-07-16 serial chain finished through Task #1727); the resulting Promotion Candidate went stale before Production review. **Replacement candidate requalification in progress under #2775** (2026-08-03). Promotion to `main` and program close remain Bill/ChatGPT authority.
...
- Priority #1 remains parked until Bill/ChatGPT explicitly launch or reprioritize it.
...
- Priority #3 is **Implementation Active** as #1719 with child tasks #1720 through #1727 under continuous reduced-gate serial authorization.

## Prioritized working backlog inventory

Items are sorted **top-down by current priority need** as of the Bill/ChatGPT PMO July 2026 review and subsequent readiness updates. Priority guides discussion and preparation order; **production-ready documentation plus explicit launch authorization remain the gates for execution.**
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | **Website Completion / Fan Club Product Buildout** | parked launch-control ready program | Complete authenticated Fan Club experience, backend services, content operations, content collection workflow, and design alignment. | Keep parked behind Program #1255/#1259 unless Bill/ChatGPT explicitly launch or reprioritize. | `#1678`; `#1685`; `#1686`–`#1694`; `docs/ops/pmo/website-completion-fan-club-product-buildout-readiness.md`; `docs/ops/implementation-plans/website-completion-fan-club-product-buildout.md` | Priority #1 group is documented and has a parked issue chain; execution remains blocked until launch authorization. | launch-control ready; blocked until queue authorization |
| 1 successor | **Website Public Launch / Relaunch Readiness** | launch-control ready successor program | Prepare the structural website baseline for public relaunch after #1685 closeout, including public copy, admin club staging, social/media reliability, donation/fundraiser readiness, SEO/analytics, launch checklist, rollback, and evidence. | Keep blocked until #1685 closeout and explicit Bill/ChatGPT authorization to begin #2041. | `#2039`; `#2041`–`#2048`; `docs/ops/pmo/website-public-launch-relaunch-readiness.md`; `docs/ops/implementation-plans/website-public-launch-relaunch-readiness.md` | Successor lane to Priority #1; does not replace #1685 and does not authorize #2040 publication workflow. | launch-control ready; blocked until #1685 closeout and queue authorization |
...
| 2 | **Fundraiser / Charity Campaign Operations Buildout** | launch-control ready future program | Repeatable fundraiser operations, Givebutter integration boundary, leaderboard/winner rules, homepage promotion, donor recognition without PII exposure, and launch testing. | Keep queued behind Program #1255/#1259, parked Priority #1, and #2039 successor unless Bill/ChatGPT explicitly launch or reprioritize. | `#1696`; `#1700`; `#1701`–`#1708`; `#1379` (historical ideas source); `docs/ops/pmo/fundraiser-charity-campaign-operations-buildout-readiness.md`; `docs/ops/implementation-plans/fundraiser-charity-campaign-operations-buildout.md` | Bill/ChatGPT identified this as the Priority #2 group. Prior backlog row "Fundraiser / charity campaign operations" merged here. | launch-control ready; blocked until queue authorization |
| 3 | **PMO Governance / Workflow Automation Completion** | Replacement candidate requalification (#2775) | Consolidate PMO July 2026 authority, workflow automation design, Cursor continuation rules, PR readiness, merge/issue policy, queue/wave planning, closeout stabilization, and backlog promotion gates. | Complete #2775 resync/requalification against current `main`; then open a new Production-review issue and obtain Bill's `PRODUCTION GO` before any promotion PR; defer C-01–C-06 to new source issues. | `#1713`; `#1719`; `#1720`–`#1727`; `#2775`; `#1411`; `#1500`; `docs/ops/pmo/pmo-governance-workflow-automation-completion-readiness.md`; `docs/ops/implementation-plans/pmo-governance-workflow-automation-completion.md`; `docs/ops/reports/program-1500-queue-wave-reconciliation.md`; `docs/ops/reports/pmo-july-2026-authority-reconciliation-1720.md`; closeout report `docs/ops/reports/pmo-governance-workflow-automation-closeout-1727.md` | Groups former ranks 3–10 into one Priority #3 program. #1411 historical; #1500 reconciled by #1725 (closed complete); prior candidate `9d6db7d` went stale and is superseded by #2775. | Requalification in progress (#2775); promotion blocked pending new candidate + Production review |
...
- They may be implemented only through Priority #3 program #1719 and child issues #1720 through #1727, unless Bill/ChatGPT explicitly authorize another current source issue.
...
Priority #3 is classified as **Requalification in progress (#2775)** because the `#1720`–`#1727` documentation chain finished on `component/pmo-governance-workflow-automation` (2026-07-16), but the resulting Promotion Candidate went stale against current `main` before Production review. #2775 prepares a replacement candidate. Task `#1725` was previously closed complete and was not rerun. Repository-wide authority and program close still require Bill/ChatGPT acceptance plus Bill/ChatGPT-approved promotion to `main` after `PRODUCTION GO`.

No backlog item may execute from backlog classification alone. Cursor and Claude Code execution require a current program/task/source issue and explicit Bill/ChatGPT launch or assignment authorization; deferred workflow/CI candidates require new source issues (see `#1726` / `#1727` reports). #1719's `#1720`–`#1727` child-chain authorization is fully consumed; further #1719 work executes only under #2775 (replacement candidate requalification) or a new source issue.
...
- Items with `implementation-ready` or `launch-control ready` readiness may move first **only after** explicit ChatGPT/Bill authorization through a current open program or source issue.
...
| 2026-07-16 | Bill authorized continuous reduced-gate serial execution for Program #1719. Priority #3 moves from launch-control ready / queued to Implementation Active. Active child #1720; Task #1725 remains closed complete. |
| 2026-07-16 | Component-branch documentation chain `#1720`–`#1727` completed (skip `#1725` rerun). Priority #3 marked Construction complete (component); promotion to `main` pending Bill/ChatGPT. Closeout packet: `docs/ops/reports/pmo-governance-workflow-automation-closeout-1727.md`. |
| 2026-08-03 | Qualified component candidate `9d6db7d` found stale (22 commits ahead / 130 commits behind current `main`) before Production review. Replacement requalification prepared and released under #2775; Priority #3 moves from Construction complete (component) to Requalification in progress. |
...
1. Bill or owner approves promotion review during a PMO meeting, weekly project review, or explicit Bill/ChatGPT review.
2. ChatGPT writes or identifies a repository design/readiness source with **production-ready documentation**.

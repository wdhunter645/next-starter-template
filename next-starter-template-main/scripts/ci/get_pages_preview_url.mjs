/*
  Find the Cloudflare Pages preview URL for a PR using GitHub API + GITHUB_TOKEN.

  Order:
  1) PR issue comments (Cloudflare bot commonly posts preview URL)
  2) Check runs output on the PR head SHA
  3) Deployments/status metadata (best-effort)
...
async function main() {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      const u = await tryFromComments();
      if (u) { process.stdout.write(u); return; }
    } catch (_) {}

    try {
      const u = await tryFromCheckRuns();
      if (u) { process.stdout.write(u); return; }
    } catch (_) {}

    try {
      const u = await tryFromDeployments();
      if (u) { process.stdout.write(u); return; }
    } catch (_) {}

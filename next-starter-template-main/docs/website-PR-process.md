...
## GitHub Label Prerequisite (Mandatory)

Intent labels (`change-ops`, `infra`, `docs-only`, `feature`, `platform`, `codex`, `recovery`) are **repository configuration** (GitHub settings), not code.

- CI and the intent labeler assume these labels already exist.
- PRs cannot create labels.
- If labels are missing, CI can fail even when the code changes are correct.

Create/verify labels before relying on governance CI.

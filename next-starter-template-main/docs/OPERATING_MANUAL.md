...
## Governance Deadlock Recovery

If PRs fail due to governance rules (not code defects):
1) Verify canonical intent labels exist in GitHub.
2) Verify `.github/intent-labeler.json` matches `scripts/ci/pr_intent_allowlists.json`.
3) Prefer ZIP-first governance repairs when multiple CI rules must be updated together.
4) Re-run checks after governance alignment; do not churn PR intent labels to “make it pass.”

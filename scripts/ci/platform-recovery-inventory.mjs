...
export const RECOVERY_ASSETS = Object.freeze([
  {
    ...
    isolatedTestPlan: 'Completed via #2896 source reconstruction path presence + config fingerprint on component tip; disposable worktree clone optional',
    testedStatus: 'tested',
  ...
  },
  {
    ...
    isolatedTestPlan: 'Completed via #2896 reconstruct count of `.github/workflows/*.yml`; live deploy not executed',
    testedStatus: 'tested',
    zeroCostBaseline: true,
  },
  {
    ...
    isolatedTestPlan: 'Completed via #2896 offline `wrangler.toml` parse (binding/DB/out dir); live dashboard confirm deferred',
    testedStatus: 'tested',
    zeroCostBaseline: true,
  },
  {
    ...
    isolatedTestPlan: 'Completed via #2896 reconstruct path presence for `functions/`; no Production deploy',
    testedStatus: 'tested',
    zeroCostBaseline: true,
  },
  {
    ...
    backupMethod: 'Provider durability + planned `wrangler d1 export` / authorized export scripts (isolated synthetic proven in #2895)',
    ...
    rto: '8 hours (target until measured live)',
    credentialBoundary: 'CLOUDFLARE_API_TOKEN + account/database ids; export artifacts must be redacted/access-controlled',
    isolatedTestPlan: 'Completed via #2895 synthetic export → local SQLite restore → schema/app probes → cleanup; live CF D1 deferred',
    testedStatus: 'tested',
    zeroCostBaseline: true,
  },
  {
    ...
    isolatedTestPlan: 'Completed via #2896 reconstruct count of `migrations/*.sql` presence; isolated apply of migrations remains separately scoped; known prefix collisions 0020/0028/0044 recorded as debt',
  ...
  },
  {
    ...
    backupMethod: 'Provider object durability + version/lifecycle assumptions (synthetic catalog integrity proven in #2895; live list deferred)',
    ...
    rto: '24 hours (target until measured live)',
    credentialBoundary: 'B2_KEY_ID / B2_APP_KEY / endpoint / bucket names only',
    isolatedTestPlan: 'Completed via #2895 synthetic catalog integrity sample + orphan detection; no Production deletes; live ListObjects deferred',
    testedStatus: 'partial',
    zeroCostBaseline: true,
  },
  {
    ...
    owner: 'Implementation / Operations',
    backupMethod: 'Git for tooling; D1 rows for catalog; B2 for objects',
    ...
    isolatedTestPlan: 'Completed via #2895 synthetic D1↔B2 key/etag reconcile on isolated restore; live dry-run sync deferred',
    testedStatus: 'tested',
    zeroCostBaseline: true,
  },
  {
    ...
    isolatedTestPlan: 'Completed via #2896 synthetic immutable candidate identity + rollback decision path + return-to-service checks; live Cloudflare Pages rollback deferred',
    testedStatus: 'tested',
    zeroCostBaseline: true,
  },
  {
...
export function runPlatformRecoveryInventory(opts = {}) {
  ...
  checks.push({
    ...
    detail: `${untestedOrPartial.length}/${RECOVERY_ASSETS.length} assets not fully restore-tested (expected for task 001)`,
    severity: 'info',
  });

...
wrangler_exec() {
  ...
  if [[ "$mode" == "command" ]]; then
    # Try --json first; if unsupported, fallback
...
if echo "$SCHEMA_OUT" | jq -e '.' >/dev/null 2>&1; then
  # Most common: { "success": true, "result": [ { results: [...] } ] } OR { "results": [...] }
...
if [[ -z "$COLS" ]]; then
  # Fallback: try to scrape "name" fields from any JSON-ish output
...
# Extract key values from wrangler output
: > "$EXISTING_KEYS_FILE"
...
if [[ ! -s "$EXISTING_KEYS_FILE" ]]; then
  # Fallback: scrape quoted strings (last resort)
...
while IFS= read -r external_id; do
  ...
  # Idempotent insert without needing UNIQUE constraint:
...
log "Generated SQL file with $NEW_COUNT statements"

# ----------------------------------------------------------------------------
# Step 5: Execute SQL (or dry-run)
# ----------------------------------------------------------------------------
if [[ "$DRY_RUN" == "1" ]]; then
...
if ! SQL_OUT="$(wrangler_exec "file" "$SQL_FILE")"; then

...
/*
  High-signal / low-brittleness invariants for LGFC.
  These are intended to catch real regressions and omissions on deployed sites.
*/
...
test("Auth-gated routes redirect to / when logged out", async ({ page }) => {
  ...
  expect(page.url().endsWith("/") || page.url().includes("/?"), `Expected redirect to /, got ${page.url()}`).toBeTruthy();
